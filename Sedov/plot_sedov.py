...
HighRes = np.load('Data/sedov_res_density.npy') #Already interpolated, 200 Points
ml_data = np.load('Data/sedov_active_density_N%i.npy'%N) #NN model data
base_data = np.load('Data/sedov_base_density.npy') #Base model data
...
if SAVE:
    plt.savefig("Results/sedov_base_density.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
ax.spines["bottom"].set_visible(False)
if SAVE:
    plt.savefig("Results/sedov_active_density.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()


ml_data_av = np.load("Data/sedov_active_av_N%i.npy"%N)
base_data_av = np.load("Data/sedov_base_av.npy")
...
ax.spines["bottom"].set_visible(False)
if SAVE:
    plt.savefig("Results/sedov_base_av.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
ax.spines["bottom"].set_visible(False)
if SAVE:
    plt.savefig("Results/sedov_active_av.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
plt.title("Sedov Relative Error",fontsize=18)
plt.legend()
plt.ylabel('Relative Error',fontsize=14)
if SAVE:
    plt.savefig("Results/sedov_error.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
if WRITE:
    with open('Results/sedov_error.txt','w') as outfile:
