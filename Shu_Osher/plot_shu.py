...
HighRes = np.load('Data/shu_res_density.npy') #Already interpolated, 200 Points
ml_data = np.load('shu_active_density_N%i.npy'%N) #NN model data
base_data = np.load('shu_base_density.npy') #Base model data

NumVals = len(HighRes)
Npts = 200
L = 10.0

dom = np.linspace(-L/2,L/2,200)
...
if SAVE:
    plt.savefig("Results/shu_density.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()

ml_data_av = np.load("Data/shu_active_av_N%i.npy"%N)
base_data_av = np.load("Data/shu_base_av.npy")
...
if SAVE:
    plt.savefig("Results/shu_av.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
plt.title("Shu-Osher Problem Relative Error")plt.legend()
plt.ylabel('Relative Error')
if SAVE:
    plt.savefig("Results/shu_error.png",dpi=600,bbox_inches='tight',transparent=True)
else:
    plt.show()
plt.clf()
...
if WRITE:
    with open('Results/shu_error.txt','w') as outfile:
